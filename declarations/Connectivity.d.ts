...
declare class Connectivity {
	 ...
	 * 
	 * ```ts
	 * let connect = new Connectivity("https://google.com");
	 * Logger.Log("Length: " + connect.getLength());
	 * ```
	 * 
	 ...
	 */
	setCallback(callback: Connectivity.Callback): void;
...
declare namespace Connectivity {
	...
	interface Callback {
		 ...
		 */
		onUrlChanged(network: Connectivity, url: Object | null): void;
		
		/**
		 * Called when any connection is opened.
		 * Usually during any processes.
		 */
		onCreateConnection(network: Connectivity, connection: Object | null): void;
		
		/**
		 * Called during connection.
		 */
		onConnect(network: Connectivity, connection: Object | null): void;
		
		/**
		 * Called during disconnection.
		 */
		onDisconnect(network: Connectivity, connection: Object | null): void;
		
		/**
		 * Called if a stream size has been requested somewhere.
		 * Especially useful not ask for size again during process.
		 */
		getLength(network: Connectivity, length: number): void;
	}
	...
	class Reader extends Connectivity {
		 ...
		 * 
		 * ```ts
		 * let reader = new Connectivity.Reader();
		 ...
		 * 
		 * ```ts
		 * let reader = new Connectivity.Reader("https://google.com");
		 * Logger.Log(reader.read());
		 * ```
		 * 
		 * @param address address
	...
	namespace Reader {
		...
		interface Callback extends Callback {
			 ...
			 */
			onCharsetChanged(reader: Connectivity.Reader, charset: string | null): void;
			
			/**
			 * Called before starting reading process.
			 */
			onPrepare(reader: Connectivity.Reader): void;
			
			/**
			 * Called when a line is read from a stream.
			 * Result in this case is currently read result.
			 */
			onReadLine(reader: Connectivity.Reader, result: Array, line: string): void;
			
			/**
			 * Called when a read is complete.
			 */
			onComplete(reader: Connectivity.Reader): void;
		}
	}
	...
	namespace Writer {
		...
		interface Callback extends Callback {
			 ...
			 */
			onPrepare(writer: Connectivity.Writer, length: number): void;
			
			/**
			 * Called during process execution, use to update data.
			 * Don't overload this method, otherwise it may lead an error.
			 */
			onProgress(writer: Connectivity.Writer, progress: number, length: number): void;
			
			/**
			 * Called when writing process has completed.
			 */
			onComplete(writer: Connectivity.Writer, length: number): void;
		}
	}
	...
	class Downloader extends Writer {
		 ...
		 * 
		 * ```ts
		 * let writer = new Connectivity.Downloader();
		 ...
		 * 
		 * ```ts
		 * let writer = new Connectivity.Downloader("https://google.com");
		 * writer.setPath("/sdcard/google.html");
		 * writer.download();
		 * ```
		 * 
		 ...
		 * 
		 * ```ts
		 * let writer = new Connectivity.Downloader("https://google.com", "/sdcard/google.html");
		 * writer.download();
		 * ```
		 * 
		 * @param address address
	...
	namespace Downloader {
		...
		interface Callback extends Writer.Callback {
			 ...
			 */
			onFileChanged(downloader: Connectivity.Downloader, file: Object | null): void;
		}
	}
