...
Environment.prototype.prepareScriptable = function () {
    var javascript = org.mozilla.javascript.Context.enter();
    javascript.setLanguageVersion(200);
    this.context = javascript;
    if (this.scriptable === undefined) {
        var scriptable = javascript.initStandardObjects(null, false);
        scriptable.variable = {};
        scriptable.query = {};
...
Environment.Parser.prototype.parse = function (stroke) {
    var formation = [], bufferDefault = null, bufferRuntime = null;
    for (var i = 0; i < stroke.length; i++) {
        var char = stroke.charAt(i);
        if (char == "`") {
            if (bufferRuntime) {
...
Environment.Sequence.prototype.process = function () {
    var task = this.getTask(), sequence = this.getSequence();
...
Environment.Task.parseInstance = function (environment, json) {
    if (json != null && typeof json == "object") {
        if (json.hasOwnProperty("type") && json.type !== undefined) {
            var formation = parseInRuntime(environment, json.type), instance = this.requireInstance(json.type);
...
Environment.Task.parseJson = function (environment, json) {
    var instance = this.parseInstance(environment, json);
...
Environment.Task.Append.prototype.getPath = function () {
    var prefix = this.getEnvironment().getPath(), path = instanceOrFormat(this.path) || null;
    return Files.restorePathCorrection(prefix, path);
};
Environment.Task.Append.prototype.getOutput = function () {
    var prefix = this.getEnvironment().getPath(), path = instanceOrFormat(this.output) || null;
    return Files.restorePathCorrection(prefix, path);
};
...
Environment.Task.Pack.prototype.process = function () {
    var who = Files.listFiles(this.getPath(), this.isMustExplore());
...
EXPORT("executeInEnvironment", function (who, scope) {
    var environment = new Environment(who);
    if (scope !== undefined) {
        environment.prepareScriptable();
        var scriptable = environment.getScriptable();
        for (var element in scope) {
