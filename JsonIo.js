...
writeNativeObjectToJsonIo = function (object, primitiveArrays) {
    ...
    var json = new JsonIo.Object();
    for (var element in object) {
        var someone = writeScriptableToJsonIo(object[element], primitiveArrays);
...
writeNativeArrayToJsonIo = function (array, primitiveArrays) {
    ...
    for (var i = 0; i < array.length; i++) {
        var someone = writeScriptableToJsonIo(array[i], primitiveArrays);
...
readJsonIoToNativeObject = function (json, allowClasses) {
    ...
    var iterator = json.keySet().iterator(), object = {};
    while (iterator.hasNext()) {
        var key = iterator.next(), someone = readJsonIoToScriptable(json.get(key), allowClasses);
...
readJsonIoToNativeArray = function (json, allowClasses) {
    ...
    if (json instanceof java.util.List || json instanceof java.util.Collection) {
        for (var i = 0; i < json.size(); i++) {
            var someone = readJsonIoToScriptable(json.get(i), allowClasses);
            someone !== undefined && array.push(someone);
        }
    }
    else {
        for (var i = 0; i < json.length; i++) {
            var someone = readJsonIoToScriptable(json[i], allowClasses);
            someone !== undefined && array.push(someone);
        }
    }
    return array;
...
writeJsonIoToStream = function (stream, json, args) {
    ...
    var writer = getJsonIoWriter(stream, args);
    writer.write(json);
    writer.flush();
    try {
        writer.close();
...
readJsonIoFromStream = function (streamOrString, args) {
    var reader = getJsonIoReader(streamOrString, args), json = reader.readObject();
    try {
        reader.close();
    }
    catch (e) {
...
readScriptableFromStream = function (streamOrString, args, allowClasses) {
    var json = readJsonIoFromStream(streamOrString, args);
